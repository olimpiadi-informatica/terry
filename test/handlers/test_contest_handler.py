...
class TestContestHandler(unittest.TestCase):
    ...
    def test_update_user_score(self):
        ...
        Database.c.execute("SELECT score FROM user_tasks WHERE token = 'token' AND task = 'poldo'")
    ...
    def _insert_data(self, token="token", task="poldo"):
        try:
            Database.c.execute("INSERT INTO tasks (name, title, statement_path, max_score, num) "
                               "VALUES ('%s', '', '', 42, 0)" % task)
        except: pass
        try:
            Database.c.execute("INSERT INTO users (token, name, surname) VALUES ('%s', '', '')" % token)
        except: pass
        try:
            Database.c.execute("INSERT INTO user_tasks (token, task, score) VALUES ('%s', '%s', 0)" % (token, task))
        except: pass
